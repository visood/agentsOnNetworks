...
object Graph{
  ...
  def degrees(al: AL): Map[Int, Int] = al.map{ case (x, nx) => (x, nx.length)}.toMap

	def emptyAL: AL = Map[Int, IndexedSeq[Int]]().withDefault( (i: Int) => IndexedSeq[Int] () )
  def nonbrAL(vs: IndexedSeq[Int]): AL = ( (for(v <- vs) yield(v, IndexedSeq[Int]())) ).toMap
	def makeLink(x: Int, y: Int): (Int, Int) = if (x < y) (x, y) else (y, x) //what is x == y?
	
	def linksAdded(al: AL)( links: (Int, Int)*): AL = 
		(al /: links){ case (al1, (x, y)) => al1 ++ Seq( x -> (al1(x) :+ y), y -> (al1(y) :+ x))}

  def linksRmvd(al: AL)( links: (Int, Int)*): AL = {
    adjListForLinks(links.toIndexedSeq)
    (al /: links){case (al1, (x, y)) => al1 ++ Seq(x -> al1(x).filter(_ != y), y -> al1(y).filter(_!= x))}
  }


	def linksERRGgivenProb( N: Int, p: Double): IndexedSeq[ (Int, Int) ] = 
		for { i <- 0 to N; j <- i + 1 until N; if (nextDouble < p )} yield makeLink(i, j)

	def adjListERRGgivenProb( N: Int, p: Double): AL = linksAdded(nonbrAL( Range(0, N) ))( linksERRGgivenProb(N, p):_*)
  ...
  def adjListERRGgivenNumber(N: Int, M: Int): AL = linksAdded(nonbrAL( Range(0, N) ))( linksERRGgivenNumber(N, M):_*)
	
	def linksInAdjList(al: AL): IndexedSeq[(Int, Int)] = 
		( Set[(Int, Int)]() /: al){ case (links, (x, nxs)) => links ++ nxs.map(makeLink(x, _))}.toIndexedSeq
	
  def adjListForLinks(links: IndexedSeq[(Int, Int)]): AL = linksAdded(nonbrAL( vertexes(links))) ( links:_*)
