...
import breeze.stats.distributions._
	...
	/** 
		...
		*/
object Gkantsidis0 extends App{
  ...
  def edgesToSwap(al1: AL, al2: AL): Option[((Int, Int), (Int, Int))] = {
    (for{ e1 <- randomLinkOnCycle(al1); e2 <- randomLink(al2)} yield (e1, e2)) match{
                            case None => (for{e1 <- randomLink(al1); e2 <- randomLinkOnCycle(al2)} yield (e1, e2))
  ...
  def connected( al: AL): Option[AL] = components(al).sortBy{ case c => - numLinksOnCycle( projection(al)(c) )} match {
    case Nil        =>  None
    case c1 :: cmps =>  ( (Some(projection(al)(c1)): Option[AL]) /: cmps) { case (None, _) => None;
  ...
  def mcmc(al: AL)(T: Int): AL = {
	...
	object tests {
		def cmptOverlap( al: AL): Boolean = {
			...
			if ((for{b <- cs; c <- cs; if (c != b) } yield ( b.intersect(c).size) ).sum == 0) true else false
		}

    def sameAdjLists(al1: AL, al2: AL): Boolean = {
      if (al1.keys != al2.keys) false
      else { al1.find{ case (x, y) => al2(x) != y} match { case None => true; case Some(_) => false } }
    }

    def sameDegrees( ds1: Map[Int, Int], ds2: Map[Int, Int]): Boolean = {
      if (ds1.keys != ds2.keys) false
      else { ds1.find{ case (x, k) => ds2(x) != k} match { case None => true; case Some(_) => false } }
    }
    ...
    def cnctdVtxOverlap( al: AL, c1: Set[Int], c2: Set[Int]): Boolean = {
      ...
      connectTwo(al1, al2) match {
        case None       => println(" seems there were no links that could connect "); true
        case Some(al12) =>  ( vertexes(al1) ++ vertexes(al2)).toSet == vertexes(al12).toSet
      }
    }
    ...
    //may be this test should be in Graph
      ...
      connectTwo(al1, al2) match {
        case None       =>  println(" seems there were no links that could connect "); true
        case Some(al12) =>  sameDegrees( degrees(al1) ++ degrees(al2), degrees(al12) )
      }
