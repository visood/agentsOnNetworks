...
object GraphTypeClass{
  ...
  trait GraphLike[V, G] {
    ...
    def from(al: Map[V, IndexedSeq[V]])(implicit evE: Link.TypeClass[V]): G = {
  ...
  object GraphLike{
    ...
    trait AdjList[V] extends GraphLike[V, AL[V]] {
      ...
      def adjList(g: AL[V]): Map[V, IndexedSeq[V]] = {
        g.asInstanceOf[Map[V, IndexedSeq[V]]]
      }
    ...
    object AdjList{
      ...
      trait Undirected[V] extends AdjList[V] 

      object Undirected {
        object OnInts extends Undirected[Int]
      }

      trait Directed[V] extends AdjList[V] 
